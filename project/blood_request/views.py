...
from .models import (
    ...
    TaskAutomationRule, Donation, VolunteerRequest, Expense, NGORegistration, BloodBankRegistration,
    BloodCampRSVP, BloodAlertSubscription
)
...
def search_donors(request):
    """Public donor search used by the Blood Donation page.

    Only donors who are currently available and have explicitly consented to
    being contacted are returned.
    """
    if getattr(request, 'limited', False):
        return JsonResponse({'success': False, 'error': 'Too many searches. Please try again shortly.'}, status=429)

    blood_group = request.GET.get('blood_group', '').strip()
    city = request.GET.get('city', '').strip()

    if not blood_group or not city:
        return JsonResponse({'success': False, 'error': 'Blood group and city are required.'}, status=400)

    donors = (
        BloodDonor.objects
        .filter(blood_group=blood_group, city__icontains=city, available_to_donate=True, consent_given=True)
        .order_by('-score', '-created_at')[:20]
    )

    results = []
    for donor in donors:
        contact_phone = donor.whatsapp_number or donor.phone
        results.append({
            'name': donor.name,
...
def certificate_lookup(request):
    """Return donor data for the client-side downloadable certificate.

    Lookup accepts either a donor DIN or registered mobile number. The latest
    Donation record is preferred for the donation date; when a donor has no
    Donation record yet, the donor registration date is used as the recognition
    date so the certificate feature remains useful for demo/test records.
    """
    query = (request.GET.get('q') or '').strip()
    if not query:
        return JsonResponse({'success': False, 'error': 'Enter your DIN or registered mobile number.'}, status=400)

    donor = BloodDonor.objects.filter(din__iexact=query).first()
    if donor is None:
        donor = BloodDonor.objects.filter(phone=query).first()
    if donor is None:
        donor = BloodDonor.objects.filter(whatsapp_number=query).first()

    if donor is None:
        return JsonResponse({'success': False, 'error': 'No donor record found. Please check your DIN or registered mobile number.'}, status=404)

    latest_donation = donor.donations.order_by('-date', '-created_at').first()
    donation_date = latest_donation.date if latest_donation else donor.last_donation_date
    date_label = 'Donation Date' if donation_date else 'Registration Date'
    if donation_date is None:
        donation_date = donor.created_at.date()

    return JsonResponse({
        'success': True,
        'donor_name': donor.name,
        'blood_group': donor.blood_group,
        'donation_date': donation_date.strftime('%d %B %Y'),
        'date_label': date_label,
        'din': donor.din,
        'units': latest_donation.units if latest_donation else 1,
        'has_donation_record': bool(latest_donation or donor.last_donation_date),
    })
    ...
    if request.method == 'POST':
        try:
            ...
            blood_request = BloodRequest.objects.create(
                ...
                units=max(1, min(int(data.get('units', 1)), 20)),
                din=din,
            )
            return JsonResponse({
                'success': True,
                'message': f'Blood request submitted successfully! Your request DIN is {din}.',
                'din': din,
            })
        except Exception as e:
            return JsonResponse({'success': False, 'error': str(e)}, status=500)
...
# ---------------------------------------------------------------------------
# Blood Donation partner / camp / alert APIs
# ---------------------------------------------------------------------------
def _clean_phone(value):
    return ''.join(c for c in str(value or '') if c.isdigit() or c == '+')
...
def ngo_register(request):
    ...
    if len(ngo_name) < 2 or len(registration_number) < 2:
        return JsonResponse({'success': False, 'error': 'NGO name and registration number are required.'}, status=400)

    if NGORegistration.objects.filter(registration_number__iexact=registration_number).exists():
        return JsonResponse({'success': False, 'error': 'This NGO registration number is already registered.'}, status=400)

    record = NGORegistration.objects.create(
        ngo_name=ngo_name,
    ...
    return JsonResponse({'success': True, 'message': 'NGO registration submitted successfully.', 'id': record.id})


def bloodbank_register(request):
    if request.method != 'POST':
        return JsonResponse({'success': False, 'error': 'Invalid request method.'}, status=405)
    data = _json_body(request)
    if data is None:
    ...
    name = str(data.get('blood_bank_name', '')).strip()
    if len(name) < 2:
        return JsonResponse({'success': False, 'error': 'Blood bank name is required.'}, status=400)

    license_number = str(data.get('license_number', '')).strip()
    if license_number and BloodBankRegistration.objects.filter(license_number__iexact=license_number).exists():
        return JsonResponse({'success': False, 'error': 'This license number is already registered.'}, status=400)

    record = BloodBankRegistration.objects.create(
        blood_bank_name=name,
        license_number=license_number,
        organization_name=str(data.get('organization_name', '')).strip(),
        city=str(data.get('city', '')).strip(),
        phone=_clean_phone(data.get('phone', '')),
        available_blood_types=str(data.get('available_blood_types', '')).strip(),
    )
    return JsonResponse({'success': True, 'message': 'Blood bank registration submitted successfully.', 'id': record.id})


def camp_rsvp(request):
    if request.method != 'POST':
        return JsonResponse({'success': False, 'error': 'Invalid request method.'}, status=405)
    data = _json_body(request)
    if data is None:
    ...
    if len(camp_name) < 2 or len(name) < 2 or len(phone) < 10:
        return JsonResponse({'success': False, 'error': 'Camp, name and valid mobile number are required.'}, status=400)
    if blood_group and blood_group not in dict(BloodDonor.BLOOD_GROUP_CHOICES):
        return JsonResponse({'success': False, 'error': 'Invalid blood group.'}, status=400)
    if BloodCampRSVP.objects.filter(camp_name=camp_name, phone=phone).exists():
        return JsonResponse({'success': False, 'error': 'You have already reserved a slot for this camp.'}, status=400)

    record = BloodCampRSVP.objects.create(
        camp_name=camp_name,
        name=name,
        phone=phone,
        blood_group=blood_group,
    )
    return JsonResponse({'success': True, 'message': 'Your camp slot has been reserved successfully.', 'id': record.id})


def alerts_subscribe(request):
    if request.method != 'POST':
        return JsonResponse({'success': False, 'error': 'Invalid request method.'}, status=405)
    data = _json_body(request)
    if data is None:
    ...
    if len(phone) < 10 or not city or blood_group not in dict(BloodDonor.BLOOD_GROUP_CHOICES):
        return JsonResponse({'success': False, 'error': 'Valid mobile number, blood group and city are required.'}, status=400)
    if not whatsapp and not sms:
        return JsonResponse({'success': False, 'error': 'Please select WhatsApp or SMS alerts.'}, status=400)

    record, created = BloodAlertSubscription.objects.update_or_create(
        phone=phone,
        defaults={
            'blood_group': blood_group,
            'city': city,
    ...
    message = 'Alerts subscription created successfully.' if created else 'Your alert preferences have been updated successfully.'
    return JsonResponse({'success': True, 'message': message, 'id': record.id})
