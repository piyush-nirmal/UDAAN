...
class Blog(models.Model):
    ...
    def display_title(self):
        ...
        # 1) Remove image extensions if they are present anywhere at the end
        #    (handles ".jpg", ".Jpg", ".JPEG", even chained like ".jpg.png")
        title = re.sub(
            r'(?:\.(?:jpg|jpeg|png|webp|gif|avif))+\s*$',
            '',
            title,
            flags=re.IGNORECASE
        ).strip()

        # 2) Remove Django's random duplicate-file suffix(es).
        #    Django appends "_XXXXXXX" (7 random alnum chars) to a filename
        #    when a file with the same name already exists. When the file
        #    name gets converted into a title (dashes/underscores -> spaces,
        #    then .title()-cased), this suffix ends up looking like an extra
        #    "word" at the end, e.g. "What Is Jal Jeevan Mission Ptbtwqx".
        #    Loop in case a file was uploaded more than once and picked up
        #    more than one suffix.
        #    Example: "What Is Jal Jeevan Mission Ptbtwqx" -> "What Is Jal Jeevan Mission"
        while True:
            new_title = re.sub(r'[\s_]+[A-Za-z0-9]{7}$', '', title)
            if new_title == title:
                break
...
class NGORegistration(models.Model):
    ...
    def __str__(self):
        return f"{self.ngo_name} ({self.status})"
...
class BloodBankRegistration(models.Model):
    ...
    def __str__(self):
        return f"{self.blood_bank_name} ({self.status})"


class BloodCampRSVP(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=['camp_name', 'phone'],
                name='unique_camp_rsvp_phone',
            )
        ]

    def __str__(self):
        return f"{self.name} - {self.camp_name}"


class BloodAlertSubscription(models.Model):
    ...
    def __str__(self):
        return f"{self.phone} - {self.blood_group} - {self.city}"
